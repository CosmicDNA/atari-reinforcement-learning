...
get_latest_model() {
    ...
    if [ -z "$LATEST_EXP_DIR" ]; then
        echo "Error: No experiment folder found for '$ALGO' on '$ENV' in '$LOG_FOLDER'." >&2
        echo "Please run a training session first with './scripts/train.sh'." >&2
        exit 1
    fi
    ...
    if [ -f "$FINAL_MODEL_PATH" ]; then
        ...
        echo "Error: No model file found in '$LATEST_EXP_DIR' to resume training." >&2
        exit 1
    fi
}

# --- Find the latest model to resume training ---
TRAINED_AGENT=$(get_latest_model)

echo "Resuming training from: $TRAINED_AGENT"

# Run the training command and pass the path to the last saved model.
# The `--trained-agent` flag tells the script to load an agent and continue training.
